...
float4 downSample4x4Glow_ps( VertexOutput_4uv IN ) : COLOR0
{
	...
	avgColor *= 0.25;
    float glowFactor = 1 - avgColor.a;
    float bloomFactor = bloomParams.x * pow(max(max(avgColor.r, avgColor.g), avgColor.b), bloomParams.y);
	return float4(avgColor.rgb, 1) * max( glowFactor, bloomFactor ); 
}
...
float4 gauss(float samples, float2 uv)
{
	...
	float4 result = tex2D(Texture0, uv) * sigmaN1;
...
{
	float3 pos = float3(coord * Params4.xy, -1);
	return normalize(pos.rgb);
}
