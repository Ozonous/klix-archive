...
#define CFG_WATER_DEPTH 250
#define CFG_WATER_DENSITY 0.3
...
// #define CFG_WATER_COLOR (13, 84, 92)
...
{
	return saturate(0.78 - 2.5 * abs(ndotv)) + 0.3;
}
...
float3 getRefractedColor(float4 cpos, float3 N, float3 waterColor)
{
    ...
    return lerp(result.rgb, waterColor, saturate((result.w - cpos.w) * (1.f / CFG_WATER_DEPTH) + CFG_WATER_DENSITY));
}
...
#ifdef CFG_WATER_COLOR
	float3 waterColor = float3 CFG_WATER_COLOR / 255.f;
#else
